...
def main():
    ...
    # 1. Master dataset containing both 500 and 1000 metrics
    df_priced.to_csv(config.DATASET_FINALE_CSV, index=False, encoding="utf-8-sig")

    # 2. Dedicated 500 credits dataset
    ...
    # 3. Dedicated 1000 credits dataset
    ...
    print("\n  SAMPLE RE-CALIBRATED FAIR AUCTION PRICES (1,000 cr vs 500 cr):")
    ...
    for _, r in sub.iterrows():
        print(f"    [{r['role']}] {r['player']:<16} Sq:{str(r['team']):<4} Fair 1000:{r['prezzo_fair_1000']:>3}cr | Fair 500:{r['prezzo_fair_500']:>3}cr (Listone:{int(r['Prezzo_Consigliato_Cr']):>2}cr)")
